...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0, p=0.5):
    ...
    #W2_rand = np.random.rand(*W2.shape)
    #dhidden = np.dot(dscores, W2_rand.T)
  ...
  def loss2(self, X, y=None, reg=0.0, p=0.5):
    ...
    loss = np.sum(margins)
    loss /= N 
    loss += 0.5 * reg * np.sum(W2 * W2) + 0.5 * reg * np.sum(W1 * W1) # regularize 
  ...
  def nesterov_momentum(self, learning_rate, grads) :
    ...
    mu = 0.9
    v1, v2, b1, b2 = 0, 0, 0, 0   
    v_prev1, v_prev2, b_prev1, b_prev2 = v1, v2, b1, b2
  ...
  def adagrad(self, learning_rate, grads):
    ...
    cache1, cache2, cacheb1, cacheb2 = 0, 0, 0, 0
    eps = 1e-6
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      # check if you need to use linear svm or softmax in the last layer
      if svm:
  ...
  def predict(self, X):
    ...
    a1 = self.ReLU(z1) # pass through ReLU activation function
